...
let yts = null;
try { yts = require('yt-search'); } catch (e) { console.warn('yt-search not available'); }
...
const config = {
  ...
  AUTO_REPLY: 'off',
  AUTO_REPLY_MSG: "🤖 I'm currently unavailable. I'll get back to you soon!",
  AUTO_VOICE: 'off',
  AUTO_LIKE_EMOJI: ['💙', '💖', '💜', '🧡', '💛', '🤍', '🖤', '❤️', '🔍 ', '✨', '💎'],
  PREFIX: '.',
  MAX_RETRIES: 3,
...
┃💬 Auto Reply : ${currentConfig.AUTO_REPLY || 'off'}
┃🎙️ Auto Voice : ${currentConfig.AUTO_VOICE || 'off'}
...
`;
        ...
        case 'autoreply': {
          try {
            ...
            if (senderNum !== sanitized && senderNum !== ownerNum) {
              return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change auto reply.' }, { quoted: msg });
            }
            ...
            if (q === 'on' || q === 'off') {
              ...
              if (args.length > 1) {
                userConfig.AUTO_REPLY_MSG = args.slice(1).join(' ');
              }
              await setUserConfigInMongo(sanitized, userConfig);
              await socket.sendMessage(sender, { react: { text: q === 'on' ? '✅' : '❌', key: msg.key } });
              await socket.sendMessage(sender, {
                text: `✅ *Auto Reply ${q === 'on' ? 'ENABLED' : 'DISABLED'}*\n${q === 'on' ? `📝 Message: _${userConfig.AUTO_REPLY_MSG || config.AUTO_REPLY_MSG}_` : ''}`
              }, { quoted: msg });
            } else {
              await socket.sendMessage(sender, {
                text: `*📋 Auto Reply Usage:*\n\n*.autoreply on* — Enable with default message\n*.autoreply on [custom message]* — Enable with custom message\n*.autoreply off* — Disable auto reply`
              }, { quoted: msg });
            }
          } catch (e) {
        ...
        case 'autovoice': {
          try {
            ...
            if (senderNum !== sanitized && senderNum !== ownerNum) {
              return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change auto voice.' }, { quoted: msg });
            }
            ...
            if (q === 'on' || q === 'off') {
              ...
              if (args.length > 1) {
                userConfig.AUTO_VOICE_MSG = args.slice(1).join(' ');
              }
              await setUserConfigInMongo(sanitized, userConfig);
              await socket.sendMessage(sender, { react: { text: q === 'on' ? '🎙️' : '❌', key: msg.key } });
              await socket.sendMessage(sender, {
                text: `✅ *Auto Voice ${q === 'on' ? 'ENABLED' : 'DISABLED'}*\n${q === 'on' ? `🎙️ Voice reply will be sent automatically to incoming messages.` : ''}`
              }, { quoted: msg });
            } else {
              await socket.sendMessage(sender, {
                text: `*🎙️ Auto Voice Usage:*\n\n*.autovoice on* — Enable voice note auto reply\n*.autovoice on [custom message]* — Enable with custom text-to-voice message\n*.autovoice off* — Disable auto voice`
              }, { quoted: msg });
            }
          } catch (e) {
...
┃ ▸ ${prefix}kick <@mention> — Remove member
┃ ▸ ${prefix}promote <@mention> — Make admin
┃ ▸ ${prefix}demote <@mention> — Remove admin
┃
┃ 🔧 *Group Settings*
┃ ▸ ${prefix}group open — Open group (all can send)
┃ ▸ ${prefix}group close — Close group (admins only)
┃ ▸ ${prefix}group lock — Lock group settings
┃ ▸ ${prefix}group unlock — Unlock group settings
┃
┃ 🔗 *Invite & Info*
┃ ▸ ${prefix}invite — Get invite link
┃ ▸ ${prefix}revoke — Revoke invite link
┃ ▸ ${prefix}groupinfo — Show group info
┃ ▸ ${prefix}groupcreate Name|num — Create group
┃
┃ 📢 *Broadcast*
┃ ▸ ${prefix}tagall [msg] — Tag all members
┃ ▸ ${prefix}online — Show online members
┃
┃ 📋 *Utility*
┃ ▸ ${prefix}groupjid — Show group JID
┃ ▸ ${prefix}grouplist — List all groups
┃
╰━━━━━━━━━━━━━━━━━━━━━━━━⬣
> *© ${BOT_NAME_FANCY}*
`.trim();
            ...
            console.error('gcmd error:', e);
            await socket.sendMessage(sender, { text: '❌ Error showing group commands!' }, { quoted: msg });
          }
          break;
        }
...
function setupMessageHandlers(socket, sessionNumber) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      ...
      let autoTyping = userConfig.AUTO_TYPING !== undefined ? userConfig.AUTO_TYPING : (config.AUTO_TYPING || 'false');
      let autoRecording = userConfig.AUTO_RECORDING !== undefined ? userConfig.AUTO_RECORDING : config.AUTO_RECORDING;
      let autoReply = userConfig.AUTO_REPLY !== undefined ? userConfig.AUTO_REPLY : config.AUTO_REPLY;
      let autoReplyMsg = userConfig.AUTO_REPLY_MSG || config.AUTO_REPLY_MSG;
      let autoVoice = userConfig.AUTO_VOICE !== undefined ? userConfig.AUTO_VOICE : config.AUTO_VOICE;
      let autoVoiceMsg = userConfig.AUTO_VOICE_MSG || autoReplyMsg;
      ...
      // Skip auto reply/voice for own messages, groups, and newsletter
      if (!isFromMe && !from.endsWith('@g.us') && !from.endsWith('@newsletter')) {

        // Extract message body for checking if it's a command
        let body = '';
        try {
          const type = getContentType(msg.message);
        ...
        const isCmd = body && body.startsWith(userPrefix);

        // Auto Reply — send text reply to non-command incoming messages
        if (autoReply === 'on' && !isCmd) {
          try {
            await socket.sendMessage(from, { text: autoReplyMsg }, { quoted: msg });
          } catch (e) {
            console.error('Auto reply error:', e);
          }
        }

        // Auto Voice — send TTS voice note to non-command incoming messages
        if (autoVoice === 'on' && !isCmd) {
          try {
            const ttsText = encodeURIComponent(autoVoiceMsg);
            const ttsUrl = `https://translate.google.com/translate_tts?ie=UTF-8&q=${ttsText}&tl=en&client=tw-ob`;
